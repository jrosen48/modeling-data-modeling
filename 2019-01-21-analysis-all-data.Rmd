...
ds <- d %>% 
    dplyr::select(sInvented, sProcedural, sConceptual, tInitSelect, tCompare, tDiscussQ, tConnectBigIdeas, tConnectOthers, tPressExplain, condition, `Duplicate Condition`) %>% 
    map_df(replace_na, 0) %>% 
    modify_at(c(1:10), add_one)
...
data.frame(dds, class = m4$predclass) %>% 
    ...
    group_by(class) %>% 
    summarize(mean_condition = mean(condition))
```

# 5. Examining predictors of the 4-class solution - does not work well for 3 class solution

```{r, spec-sols-pred, cache = FALSE, eval = FALSE}
s <- read_rds('starts.rds')

f <- cbind(sInvented, sProcedural, sConceptual, tInitSelect, tCompare, tDiscussQ, tConnectBigIdeas, tConnectOthers, tPressExplain) ~ condition

m2 <- poLCA(f, dds, nclass = 2, maxiter = 10000, graphs = TRUE)
m3 <- poLCA(f, dds, nclass = 3, maxiter = 10000, graphs = TRUE)
m4 <- poLCA(f, dds, nclass = 4, maxiter = 10000, graphs = TRUE, probs.start = s)
m5 <- poLCA(f, dds, nclass = 5, maxiter = 10000, graphs = TRUE)
```